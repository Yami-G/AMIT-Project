...
class Validators {
  String? Function(String?)? validateMobileNumber(BuildContext context) {
    Pattern patternMobileNumber = r'^(?:[+0]9)?[0-9|٩|٠|١|٢|٣|٤|٥|٦|٧|٨]{10,15}$';
    return (value) {
      value = value?.trim();
      if (value!.isEmpty) {
  ...
  String? Function(String?)? validatePositiveInteger(BuildContext context) {
    return (value) {
      if (value!.isEmpty) {
        ...
        return "من فضلك ادخل رقم اكبر من الصفر";
      } else {
        return null;
      }
    };
  ...
  String? Function(String?)? validateCreatePassword(BuildContext context) {
    String patternEmail = r'^(?=.*?[a-z])(?=.*?[0-9])(?=.*?[!-_@\#&$%^*]).{8,32}$';
    /*
  ...
  String? Function(String?)? validateConfirmPassword(
    ...
    return (value) {
      if (value!.isEmpty) {
        ...
        return "كلمتي المرور غير متطابقتان";
      } else {
        return null;
      }
    };
